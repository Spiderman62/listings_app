...
const deleteListing = id => {
    if(confirm('Are you sure you want to delete?')) router.delete(route('listing.destroy',id))
}
</script>
<template>
    ...
    <div v-if="listings">
        <div v-if="listings.data.length > 0">
            ...
            <div class="mb-6">
                <table
                    ...
                    <tr
                        ...
                        :key="listing.id"
                        class="border-b border-slate200 hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-600 dark:border-slate-600">
                        <td class="w-3/4 p-3 text-left">
                            <div class="flex items-center gap-2">
                                ...
                                <h4 class="font-bold">{{ listing.title }}
                                    <i :class="`fa-solid fa-circle-${listing.approved ? 'check text-green-500' : 'xmark text-red-500'}`"></i>
                                </h4>
                            </div>
                        </td>
            ...
            <div>
                <PaginatorLink :paginator="listings"/>
            </div>
        </div>
        <div v-else>
