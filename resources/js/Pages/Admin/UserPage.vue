...
const toggleApprove = listing=>{
    let msg = listing.approved ? 'Disapproved this listing?': 'Approved this listing?';
    if(confirm(msg)){
...
<template>
    <Head :title="`- ${user.name}'s listings `"/>
        ...
        <div class="flex item-center justify-between">
            ...
            <div
                ...
                <input
                    ...
                    type="checkbox"
                    id="showDisapproved"
                    class="rounded-md border-1 outline-0 text-indigo-500 ring-indigo-500 accent-indigo-500 border-slate-700 cursor-pointer">
                <label for="showDisapproved"
                       class="block text-sm font-medium text-slate-700 dark:text-slate-300 cursor-pointer">Show
                    suspended users</label>
            </div>
        </div>
    </div>
    <!--  Table  -->
    <table class="bg-white dark:bg-slate-800 w-full rounded-lg overflow-hidden ring-1 ring-slate-300">
        ...
        <tr v-for="listing in user_listings.data" :key="listing.id">
            <td class="py-5 px-3">{{ listing.title }}</td>
            <td class="py-5 px-3 text-2xl text-center">
                <button @click.prevent="toggleApprove(listing)">
                    <i :class="`fa-solid fa-circle-${listing.approved ? 'check text-green-400' : 'xmark text-red-400'}`"></i>
                </button>
            </td>
    ...
    <div class="mt-6">
        <PaginatorLink :paginator="user_listings"/>
    </div>
</template>
